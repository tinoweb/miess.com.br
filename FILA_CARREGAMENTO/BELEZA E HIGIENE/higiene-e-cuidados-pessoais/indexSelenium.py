...
lista_de_produtos = []
links_processados = 0
url_categoria = 'https://www.miess.com.br/beleza-e-higiene/higiene-e-cuidados-pessoais'
    ...
    if response.status_code == 200:
        ...
        if match:
            ...
            for sku in sku_json.get('skus', []):
                ...
                aroma = sku['dimensions'].get('Aroma', None)
                if aroma:
                    if variation_name:
                        variation_name += ", "
                    variation_name += f"Aroma: {aroma}"
...
def processar_pagina(url_base):
    ...
    while current_page <= numero_de_paginas:
        ...
        produtos = soup.find_all('li', class_=cuidadosPessoais)
