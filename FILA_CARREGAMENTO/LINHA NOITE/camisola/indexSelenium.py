...
lista_de_produtos = []
links_processados = 0
url_categoria = 'https://www.miess.com.br/linha-noite/camisola'
...
noite = "linha-noite---baby-doll-camisolas-e-pijamas-sensuais-|-miess fixVar event-set-tag event-set"
...
def get_product_details(url):
    ...
    if response.status_code == 200:
        ...
        with open('hobby.html', 'w', encoding='utf-8') as file:
        ...
        if match:
            ...
            for sku in sku_json.get('skus', []):
                if sku['available']:  # Verifica se a variação está disponível
...
def extract_total_products(soup):
    try:
        # Agora, busca pelo span com a nova classe
        total_products_span = soup.find('span', class_='text-red-wine font-bold')

        if total_products_span:
            # Extrai o texto e em seguida filtra apenas os dígitos.
            total_products_text = total_products_span.get_text(strip=True)
            # Utiliza regex para encontrar apenas os números no texto.
            total_products_numbers = ''.join(filter(str.isdigit, total_products_text))

            try:
                # Converte o texto dos números para int
                total_products = int(total_products_numbers)
...
def processar_pagina(url_base):
    ...
    while current_page <= numero_de_paginas:
        ...
        # produtos = soup.find_all('li', class_=noite)
        # Encontre todos os produtos usando a nova estrutura de classe para o contêiner dos produtos
        produtos = soup.find_all('div', class_='flex flex-col h-full gap-3 w-full pb-px m-auto')
        ...
        #     link_produto_element = produto.find('a', class_='group/image')
        ...
        for produto in produtos:
            ...
            outStock = produto.find('p', class_='text-xs font-semibold text-red-wine')
            if not outStock:  # Se não encontrou a classe, significa que o produto está em estoque
                link_produto_element = produto.find('a', class_='group/image')
                if link_produto_element:
                    link_produto = link_produto_element['href']
                    titulo_produto = link_produto_element.get('title', '').strip()
                    ...
                    if detalhes:
                        detalhes['titulo'] = titulo_produto
                        detalhes['ProdutoEsgotado'] = False  # Produto está em estoque

                        lista_de_produtos.append(detalhes)
                        links_processados += 1  # Incrementar o contador
            else:
                print(f"Produto esgotado: {titulo_produto} - não será adicionado à lista.")
                # Opcional: Você pode adicionar um tratamento aqui para produtos esgotados, se necessário.
