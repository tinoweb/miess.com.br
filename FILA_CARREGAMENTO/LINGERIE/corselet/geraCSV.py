    ...
    with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for product_data in product_data_list:
            ...
            for variation in product_data["Variações do produto"]:
                # Dentro do loop, ao processar cada variação do produto
                # variant_price_str = variation["Preço da variação"]
                # # Remover os separadores de milhar e converter a vírgula em ponto
                # variant_price_str_clean = variant_price_str.replace("R$", "").replace(".", "").replace(",", ".")
                # variant_price = "{:.2f}".format(float(variant_price_str_clean.strip()))

                variant_price = "{:.2f}".format(product_data["Novo preço (150%)"])

                # Separar a variação em cor e tamanho
